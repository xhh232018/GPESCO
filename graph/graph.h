...
public:
    ...
    void loadGraphFromMemory(std::vector<std::tuple<uint32_t, uint32_t, uint32_t>> &vertex_list,
                             std::vector<std::pair<uint32_t, uint32_t>> &edge_list);


    void constuctPerturbedQueryG(const std::string& file_path, ui skip_cnt, VertexID begin_, VertexID end_);
public:
    ...
    const ui getOffset(const VertexID id) const {
        return offsets_[id];
    }
    ...
    int getNeiIdx(VertexID u, VertexID v) const {
        if (getVertexDegree(u) < getVertexDegree(v)) {
            std::swap(u, v);
        }
        ui count = 0;
    ...
    bool checkEdgeExistence(VertexID u, VertexID v, LabelID l_u, LabelID l_v) const {
        // if (getVertexDegree(u) < getVertexDegree(v)) {
        //     std::swap(u, v);
        // }
        if (u < v) {
        ...
        ui count = 0;
        //const VertexID* neighbors =  getVertexNeighbors(v, count);
        const VertexID* neighbors =  getNeighborsByLabel(v,l_v,count);
