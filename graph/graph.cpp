...
void Graph::constuctPerturbedQueryG(const std::string& file_path, ui skip_cnt, VertexID begin_, VertexID end_){//add a leaf flag
...
void Graph::loadGraphFromMemory(std::vector<std::tuple<uint32_t, uint32_t, uint32_t>> &vertex_list,
    ...
    for (uint32_t i = 0; i < vertices_count_; ++i) {
        ...
        VertexID id = i;
        LabelID label = std::get<1>(vertex_list[i]);
        ui degree = std::get<2>(vertex_list[i]);
    ...
    for (auto& edge : edge_list) {
        VertexID begin = edge.first;
        VertexID end = edge.second;
