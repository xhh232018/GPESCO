...
void
preprocessor::execute(const Graph *query_graph, const Graph *data_graph, catalog *storage, bool enable_elimination, uint32_t u, uint32_t v) {
    initialize(query_graph, data_graph);
    scan_relation(storage,u,v);
...
void preprocessor::initialize(const Graph *query_graph, const Graph *data_graph) {
    ...
    if (non_core_vertices_count_ != 0) {
        ...
        std::fill(non_core_vertices_parent_, non_core_vertices_parent_ + non_core_vertices_count_, 999);
...
void preprocessor::eliminate_dangling_tuples(catalog *storage) {
    ...
    for (uint32_t i = 0; i < non_core_vertices_count_; ++i) {
        ...
        ui deg = query_graph_->getVertexDegree(u);

        if(storage->query_graph_->getVertexDegree(u) == 0)
            continue;
        ...
        if(v > non_core_vertices_count_) continue;
        ...
        if (vertices_index_[v] < non_core_vertices_count_ && vertices_index_[v] < (vertices_count_ - 1)) {
            ...
            if(w > non_core_vertices_count_) continue;
    ...
    semi_join_time_ = bottom_up_non_core_semi_join_time_ + bottom_up_core_semi_join_time_ + top_down_non_core_semi_join_time_+ top_down_core_semi_join_time_;
...
void preprocessor::scan_relation(catalog *storage, uint32_t src, uint32_t dst) {
    auto scan_operator = new scan(data_graph_);
    ...
    uint32_t src_label = query_graph_->getVertexLabel(src);
    uint32_t dst_label = query_graph_->getVertexLabel(dst);

    scan_operator->execute(src_label, dst_label, &(storage->edge_relations_[src][dst]), true);
