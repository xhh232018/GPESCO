...
private:
    ...
    static void determine_sp_level(std::vector<nd_tree_node>& density_tree, std::vector<std::vector<uint32_t>> &vertex_orders, catalog *storage, uint32_t begin_, uint32_t end_);
    ...
    static void generate_query_plan_with_nd(Graph *query_graph, catalog *storage, std::vector<std::vector<uint32_t>>& vertex_orders, uint32_t begin_, uint32_t end_);
