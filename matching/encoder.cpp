...
void encoder::execute_iee(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan){
    ...
    for (uint32_t i = 2; i < core_vertices_cnt || i == 0; ++i) {
...
void encoder::execute_nc(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan){
...
void encoder::convert_to_encoded_relation(catalog *storage) {
    ...
    if(storage->alg == "JointEnum" || storage->alg == "LookAheadEnum" ){
        if(storage->end_depth > 0){
            if(storage->end_depth < core_vertices_cnt ){
                uint32_t u = join_plan_[storage->end_depth];
                uint32_t bn = join_plan_[storage->begin_depth];
