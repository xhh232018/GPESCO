...
void query_plan_generator::generate_query_plan_with_nd(Graph *query_graph, catalog *storage, std::vector<std::vector<uint32_t>>& vertex_orders, uint32_t begin_, uint32_t end_) {
    ...
    determine_sp_level(density_tree, vertex_orders, storage, begin_, end_);
}


void query_plan_generator::determine_sp_level(std::vector<nd_tree_node>& density_tree, std::vector<std::vector<uint32_t>> &vertex_orders, 
                                                catalog *storage, uint32_t begin_, uint32_t end_){
    std::vector<uint32_t> t_12, t_23, t_34;
    std::vector<uint32_t> order_copy = vertex_orders.back();
    uint32_t dense_idx, depth = 0;
    ...
    while (!q.empty()) {
        ...
        if(density_tree[node_id].r_ == 1 && density_tree[node_id].s_ == 2){
            t_12.emplace_back(node_id);
        }
        else if (density_tree[node_id].r_ == 2 && density_tree[node_id].s_ == 3)
        {
           t_23.emplace_back(node_id);
        }
        else{
            t_34.emplace_back(node_id);
        }
    ...
    //select densest-tree
    uint32_t t34_size_max = 0, t23_size_max = 0;

    if(t_34.size() > 0) {
        for(int i = 0; i < t_34.size(); ++i){
            if(density_tree[t_34[i]].vertices_.size() > t34_size_max){
                t34_size_max = density_tree[t_34[i]].vertices_.size();
            }
        }
    }

    if(t_23.size() > 0) {
        for(int i = 0; i < t_23.size(); ++i){
            if(density_tree[t_23[i]].vertices_.size() > t23_size_max){
                t23_size_max = density_tree[t_23[i]].vertices_.size();
            }
        }
    }
    ...
    if(t34_size_max > 3){
        for(int i = 0; i < t_34.size(); ++i){
            if(density_tree[t_34[i]].density_ > max_density && density_tree[t_34[i]].vertices_.size() > 3){
                max_density = density_tree[t_34[i]].density_;
            }
        }
        
        for(int i = 0; i < t_34.size(); ++i){
            if(density_tree[t_34[i]].density_ == max_density){
               for(auto &node:density_tree[t_34[i]].vertices_){
                    auto iter = std::find(order_copy.begin(),order_copy.end(),node);
                    uint32_t idx = iter - order_copy.begin();
                    if(idx > depth) depth = idx;
               }
            }
        }
        
    } 
    else if(t23_size_max > 3){
        for(int i = 0; i < t_23.size(); ++i){
            if(density_tree[t_23[i]].density_ > max_density && density_tree[t_23[i]].vertices_.size() > 3){
                max_density = density_tree[t_23[i]].density_;
            }
        }
        for(int i = 0; i < t_23.size(); ++i){
            if(density_tree[t_23[i]].density_ == max_density){
               for(auto &node:density_tree[t_23[i]].vertices_){
                    auto iter = std::find(order_copy.begin(),order_copy.end(),node);
                    uint32_t idx = iter - order_copy.begin();
                    if(idx > depth) depth = idx;
               }
    ...
    if(max_density == 0) {
        storage->max_depth = storage->query_graph_->get2CoreSize();
        if(storage->max_depth == 0) storage->max_depth = 2;
    }
    else{
        storage->max_depth = depth + 1;
    }
    storage->combine_core = storage->query_graph_->get2CoreSize();
}
...
void query_plan_generator::print_vertex_orders(Graph *query_graph, std::vector<std::vector<uint32_t>> &vertex_orders,
    ...
    query_graph->max_util_value = max_utility_value;
