...
protected:
    ...
    virtual void execute_operator_nc(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_je(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_lae(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_probe_mat(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_pe(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_iee(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_rwc(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_rwch(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_rwd(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_rwdw_epl(std::vector<flat_relation *> &relations) = 0;
    virtual void execute_operator_rwdw_final(std::vector<flat_relation *> &relations) = 0;
...
public:
    ...
    uint64_t execute_nc(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit) {
        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_nc(relations);
    ...
    uint64_t execute_je(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){
        
        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_je(relations);
    ...
    uint64_t execute_lae(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_lae(relations);
    ...
    uint64_t execute_probe_mat(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_probe_mat(relations);
    ...
    uint64_t execute_pe(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_pe(relations);
    ...
    uint64_t execute_iee(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_iee(relations);
    ...
    uint64_t execute_rwc(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_rwc(relations);
    ...
    uint64_t execute_rwch(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_rwch(relations);
    ...
    uint64_t execute_rwd(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_rwd(relations);
    ...
    uint64_t execute_rwdw_epl(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_rwdw_epl(relations);
    ...
    uint64_t execute_rwdw_final(catalog& catalog, std::vector<flat_relation*>& relations, uint64_t output_count_limit){

        initialize(&catalog, relations, output_count_limit);

        auto start = std::chrono::high_resolution_clock::now();

        execute_operator_rwdw_final(relations);
...
protected:
    ...
    void execute_operator_nc(std::vector<flat_relation *> &relations) override;

    void execute_operator_je(std::vector<flat_relation *> &relations) override;

    void execute_operator_lae(std::vector<flat_relation *> &relations) override;

    void execute_operator_probe_mat(std::vector<flat_relation *> &relations) override;

    void execute_operator_pe(std::vector<flat_relation *> &relations) override;

    void execute_operator_iee(std::vector<flat_relation *> &relations) override;

    void execute_operator_rwc(std::vector<flat_relation *> &relations) override;

    void execute_operator_rwch(std::vector<flat_relation *> &relations) override;

    void execute_operator_rwd(std::vector<flat_relation *> &relations) override;

    void execute_operator_rwdw_epl(std::vector<flat_relation *> &relations) override;

    void execute_operator_rwdw_final(std::vector<flat_relation *> &relations) override;
