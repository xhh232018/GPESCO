...
void encoderSP::execute_iso(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan){
    uint32_t core_vertices_cnt = storage->combine_core;
    ...
    if(!storage->iso){
        uint32_t bn = join_plan_[0];
        uint32_t u = join_plan_[1];
        convert_to_hash_relation(storage, bn, u);
    }
}

void encoderSP::execute_nc(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan){
    uint32_t core_vertices_cnt = storage->combine_core;
    join_plan_ = join_plan;
    auto projection_operator = new projection(max_vertex_id_);
    auto start = std::chrono::high_resolution_clock::now();
    ...
    //encode "end_depth"
    if(query_graph_->getVertexDegree(join_plan_[storage->end_depth]) > 0){
        uint32_t u = join_plan_[storage->end_depth];
        for(int k = storage->end_depth + 1; k < query_graph_->getVerticesCount();++k){
            uint32_t v = join_plan_[k];
    ...
    if(storage->end_depth > 0){
        if(storage->end_depth < core_vertices_cnt){
            uint32_t u = join_plan_[storage->end_depth];
            uint32_t bn = join_plan_[storage->begin_depth];
            convert_to_encoded_relation(storage, bn, u);
        }
        else{
            uint32_t u = join_plan_[storage->end_depth];
...
void encoderSP::convert_to_encoded_relation(catalog *storage, uint32_t u, uint32_t v, uint32_t index){
    ...
    for (uint32_t i = 0; i < u_candidates_cnt; ++i) {
        ...
        if(local_degree == 0) local_zero_deg[index][u_candidate] = true;

    }

    //std::cout<< "Number of nodes with zero local degree : " << zero_lcdeg_cnt << ", (u,v) = (" << u <<"," << v << ")." << std::endl;
...
void encoderSP::convert_to_hash_relation(catalog *storage, uint32_t u, uint32_t v) {
...
void encoderSP::convert_to_trie_relation(catalog *storage, uint32_t u, uint32_t v) {
    // We assume that the relation is ordered.
    uint32_t src = std::min(u, v);
    uint32_t dst = std::max(u, v);
...
void encoderSP::convert_to_encoded_relation(catalog *storage) {
    uint32_t core_vertices_cnt;
    if(storage->anc) core_vertices_cnt = storage->combine_core;
    else core_vertices_cnt = query_graph_->get2CoreSize();
    ...
    for (uint32_t i = 0; i < core_vertices_cnt || i == 0; ++i) {
        ...
        for (uint32_t j = 0; j < nbr_cnt; ++j) {
            ...
            //projection_operator->execute(&storage->edge_relations_[src][dst], kp, storage->candidate_sets_[u], storage->num_candidates_[u]);
            if(j == 0) projection_operator->init(&storage->edge_relations_[src][dst], kp);
            else projection_operator->update(&storage->edge_relations_[src][dst], kp);
        }
        projection_operator->store_res(storage->candidate_sets_[u], storage->num_candidates_[u]);
    }
    ...
    for (uint32_t i = 1; i < n; ++i) {
        ...
        for (uint32_t j = 0; j < i; ++j) {
            ...
            if (query_graph_->checkEdgeExistence(bn, u)) {
                if (i < core_vertices_cnt) {
                    convert_to_encoded_relation(storage, bn, u, j);
                }
                else {
                    convert_to_hash_relation(storage, bn, u);
                }
...
void encoderSP::refine_candidate_set(catalog *storage){
    for(int i = 0; i < core_size; ++i){
        uint32_t u = join_plan_[i];
        uint32_t valid_cnt = 0;
        uint32_t ori_cnt = storage->get_num_candidates(u);
        //uint32_t* candidates = storage->candidate_sets_[u];

        for(int j = 0; j < ori_cnt; ++j){
            uint32_t v = storage->candidate_sets_[u][j];
            if(!local_zero_deg[i][v]){
                storage->candidate_sets_[u][valid_cnt++] = v;
            }
        }

        storage->num_candidates_[u] = valid_cnt;
    }

    //reset local_zero_flag
    for(int i = 0; i < core_size;++i){
        memset(local_zero_deg[i], 0, sizeof(bool) * max_vertex_id_);
    }
}

void encoderSP::execute(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan) {
    auto start = std::chrono::high_resolution_clock::now();
    join_plan_ = join_plan;
...
void encoderSP::execute(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan, uint32_t u, uint32_t v) {
    auto start = std::chrono::high_resolution_clock::now();
    join_plan_ = join_plan;
    switch (relation_type) {
        case RelationStructure::TrieRelation:
    ...
    convert_to_hash_relation(storage,u,v);
...
void encoderSP::convert_to_encoded_relation(catalog *storage, uint32_t u, uint32_t v) {
    uint32_t src = std::min(u, v);
    uint32_t dst = std::max(u, v);
    edge_relation& target_edge_relation = storage->edge_relations_[src][dst];
    edge* edges = target_edge_relation.edges_;
...
void encoderSP::convert_trie_relation_to_sparse_bitmap(catalog *storage) {
...
void encoderSP::convert_encoded_relation_to_sparse_bitmap(catalog *storage) {
    uint32_t core_vertices_cnt = query_graph_->get2CoreSize();
...
void encoderSP::convert_hash_relation_to_sparse_bitmap(catalog *storage) {
    uint32_t core_vertices_cnt = query_graph_->get2CoreSize();
...
void encoderSP::convert_to_hash_relation(catalog *storage) {
    uint32_t n = query_graph_->getVerticesCount();
...
void encoderSP::convert_to_trie_relation(catalog *storage) {
    uint32_t n = query_graph_->getVerticesCount();
...
void encoderSP::space_init_proj(catalog *storage){

    auto projection_operator = new projection(max_vertex_id_);
    auto start = std::chrono::high_resolution_clock::now();
    for (uint32_t i = 0; i < core_size || i == 0; ++i) {
        uint32_t u = join_plan_[i];
        uint32_t nbr_cnt;
        const uint32_t* nbrs = query_graph_->getVertexNeighbors(u, nbr_cnt);
        for (uint32_t j = 0; j < nbr_cnt; ++j) {
...
void encoderSP::candidate_space_shrinking(catalog *storage){
...
void encoderSP::execute_with_shrinking(catalog *storage, RelationStructure relation_type, bool enable_sparsebp, uint32_t *join_plan, int k){
    join_plan_ = join_plan;
    auto start = std::chrono::high_resolution_clock::now();
    space_init_proj(storage);

    for(int i = 0;i < k; ++i){

        candidate_space_shrinking(storage);
    ...
    if(storage->end_depth > 0){
        if(storage->end_depth < core_size ){
            uint32_t u = join_plan_[storage->end_depth];
            uint32_t bn = join_plan_[storage->begin_depth];
            convert_to_encoded_relation(storage, bn, u);
        }
