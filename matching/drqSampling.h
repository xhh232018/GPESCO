#ifndef SUBGRAPHMATCHING_DRQSAMPLING_H
#define SUBGRAPHMATCHING_DRQSAMPLING_H
#include <chrono>
#include <future>
#include <thread>
#include <fstream>
...
public:
    ...
    Res_PQ rpq;
public:
    drqSampling(std::string tk_, std::string of, bool es, bool ee, RelationStructure rt, uint32_t k, uint64_t tl, uint64_t ol):
    tk(tk_), output_file(of), enable_sparsebp(es), enable_elimination(ee), relation_type(rt), topk(k), time_limit(tl), output_limit(ol) {}
    
    void RWD(Graph* query_graph_ori, Graph* query_graph, Graph* data_graph, uint32_t u, uint32_t v);

    void adjust_order(Graph* query_graph,std::vector<uint32_t> &order);

    void execute_within_time_limit_rwd(execution_tree* tree, catalog* catalog, uint64_t output_limit, uint64_t time_limit);

    void execute_within_time_limit_rwdw_epl(execution_tree* tree, catalog* catalog, uint64_t output_limit, uint64_t time_limit);

    void execute_within_time_limit_rwdw_final(execution_tree* tree, catalog* catalog, uint64_t output_limit, uint64_t time_limit);
    ...
    void common_label_detection(Graph* query_graph, catalog* storage,std::vector<uint32_t> &comp_small, std::vector<uint32_t> &comp_large);
    ...
    void adjust_single(Graph* component_graph,std::vector<uint32_t> &order,uint32_t end_depth,std::vector<uint32_t> &new_order,uint32_t &total_core);
    ...
    void RWDW(std::vector<uint32_t> &component_small,std::vector<uint32_t> &component_large,Graph* query_graph, Graph* data_graph, uint32_t u, uint32_t v);
};
